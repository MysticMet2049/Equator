import { FiBell, FiClock, FiHeart, FiLogOut, FiPackage, FiRefreshCw, FiUser } from "react-icons/fi";
...
const NAV_ITEMS = [
  ...
  { id: "notifs", label: "Activité", icon: FiBell },
    ...
    <aside className="lg:w-72 shrink-0">
      <div className="bg-white rounded-2xl p-5 sticky top-20" style={{ border: "1px solid var(--color-equator-beige)" }}>
        ...
        <div className="mt-6 pt-5 space-y-2" style={{ borderTop: "1px solid var(--color-equator-beige)" }}>
          <button onClick={onRefresh} disabled={loading} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm" style={{ color: "var(--color-equator-muted)", fontFamily: "var(--font-body)" }}>
            <FiRefreshCw size={16} className={loading ? "animate-spin" : ""} /> Actualiser
          </button>
          <button onClick={onLogout} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm" style={{ color: "#dc2626", fontFamily: "var(--font-body)" }}>
