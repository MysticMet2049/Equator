...
export default function StoreInfoTab({ store, isAuthenticated, showAllDesc, setShowAllDesc, whatsappUrl }) {
  return (
    <div className="pb-12 max-w-2xl">
      <div className="bg-white rounded-2xl p-6 space-y-5" style={{ border: "1px solid var(--color-equator-beige)" }}>
        <AboutBlock store={store} showAllDesc={showAllDesc} setShowAllDesc={setShowAllDesc} />
        <StoreInformation store={store} />
        <StoreContact store={store} isAuthenticated={isAuthenticated} whatsappUrl={whatsappUrl} />
      </div>
    </div>
  );
}
...
function AboutBlock({ store, showAllDesc, setShowAllDesc }) {
  const shouldCrop = store.description.length > 180;
  const displayedText = showAllDesc ? store.description : store.description.slice(0, 180) + (shouldCrop ? "..." : "");
    ...
    <Section>
      ...
      {rows.map(({ label, value }) => (
        <div key={label} className="flex justify-between py-2.5" style={{ borderBottom: "1px solid var(--color-equator-beige)" }}>
          ...
          <span className="text-sm font-medium" style={{ color: "var(--color-equator-text)", fontFamily: "var(--font-body)" }}>{formatDisplayValue(value)}</span>
        </div>
      ))}
    </Section>
  );
    ...
    <Section>
      ...
      {!isAuthenticated ? (
        ...
        <div className="space-y-2">
          {store.contact?.email && <ContactItem href={`mailto:${store.contact.email}`} icon={FiMail} label={store.contact.email} />}
          {store.contact?.phone && <ContactItem href={`tel:${store.contact.phone}`} icon={FiPhone} label={store.contact.phone} />}
          {store.contact?.whatsapp && <ContactItem href={whatsappUrl} icon={PiWhatsappLogo} label="WhatsApp" whatsapp />}

          {!store.contact?.phone && !store.contact?.email && !store.contact?.whatsapp && (
            <p className="text-sm" style={{ color: "var(--color-equator-muted)" }}>Aucun contact disponible.</p>
          )}
        </div>
      )}
    </Section>
...
function ContactItem({ href, icon: Icon, label, whatsapp = false }) {
  return (
    <a
      ...
      style={{ color: whatsapp ? "#25d366" : "var(--color-equator-text)", fontFamily: "var(--font-body)" }}
    >
      <Icon size={14} style={whatsapp ? undefined : { color: "var(--color-equator-green)" }} /> {label}
    </a>
  );
}

function Section({ children }) {
  return <div style={{ borderTop: "1px solid var(--color-equator-beige)", paddingTop: "1.25rem" }}>{children}</div>;
}
