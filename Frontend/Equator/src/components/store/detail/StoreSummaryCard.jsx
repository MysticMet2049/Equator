import { FiCheck, FiChevronDown, FiChevronUp, FiHeart, FiMail, FiMapPin, FiMessageCircle, FiPackage, FiPhone } from "react-icons/fi";
  ...
  return (
    <div className="bg-white rounded-2xl -mt-10 relative z-10 p-6" style={{ border: "1px solid var(--color-equator-beige)", boxShadow: "0 4px 24px rgba(0,0,0,0.06)" }}>
      <div className="flex flex-col sm:flex-row gap-4 items-start">
        ...
        <StoreActions followed={followed} setFollowed={setFollowed} contactOpen={contactOpen} setContactOpen={setContactOpen} />
      </div>

      {contactOpen && <StoreContactButtons store={store} isAuthenticated={isAuthenticated} whatsappUrl={whatsappUrl} />}
    </div>
  );
}
    ...
    >
      {store.logoId ? (
        <ApiImage
          ...
          type={store.logoType}
          typeCandidates={["STORE_LOGO", "ORGANISATION_LOGO", "LOGO", "IMAGE"]}
          alt={`${store.name} logo`}
  ...
  return (
    <div className="flex-1 min-w-0">
      ...
      <div className="flex flex-wrap items-center gap-4 text-xs" style={{ color: "var(--color-equator-muted)", fontFamily: "var(--font-body)" }}>
        ...
        <span className="flex items-center gap-1"><FiPackage size={11} /> {store.productCount} produits</span>
        <span className="flex items-center gap-1"><FiMapPin size={11} /> {formatDisplayValue(store.location)}</span>
        <span>Membre depuis {store.since}</span>
      </div>

      <div className="flex flex-wrap gap-1.5 mt-3">
        {store.badges.map((badge) => (
          <span key={badge} className="text-xs px-2.5 py-0.5 rounded-full font-medium flex items-center gap-1" style={{ background: "#e8f5ee", color: "var(--color-equator-green)", fontFamily: "var(--font-body)" }}>
            <FiCheck size={10} /> {badge}
          </span>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="mt-4 pt-4 border-t" style={{ borderColor: "var(--color-equator-beige)" }}>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {store.contact?.email && <ContactLink as="a" href={`mailto:${store.contact.email}`} icon={FiMail} label={store.contact.email} message="Connectez-vous pour contacter le vendeur par email." />}
        {store.contact?.phone && <ContactLink as="a" href={`tel:${store.contact.phone}`} icon={FiPhone} label={store.contact.phone} message="Connectez-vous pour appeler le vendeur." />}
