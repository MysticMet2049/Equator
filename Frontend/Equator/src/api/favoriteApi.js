...
function getMutationCandidates(kind, productId) {
  const configuredPath = templatePath(getConfiguredEndpoint(kind), { productId });
  const body = favoriteBody(productId, kind === "add");

  const defaultAddCandidates = [
    { method: "post", path: `/api/client/catalog/products/${productId}/favorite`, body },
    { method: "post", path: `/api/client/catalog/product/${productId}/favorite`, body },
    { method: "post", path: `/api/client/catalog/products/favorite-products/${productId}`, body },
    { method: "post", path: `/api/client/catalog/product/favorite-products/${productId}`, body },
    { method: "post", path: "/api/client/catalog/products/favorite-products", body },
    { method: "post", path: "/api/client/catalog/product/favorite-products", body },
    { method: "post", path: "/api/client/user-preferences", body },
    { method: "post", path: "/api/client/preferences", body },
    { method: "put", path: "/api/client/user-preferences", body },
    { method: "put", path: "/api/client/preferences", body },
  ];

  const defaultRemoveCandidates = [
    { method: "delete", path: `/api/client/catalog/products/${productId}/favorite` },
    { method: "delete", path: `/api/client/catalog/product/${productId}/favorite` },
    { method: "delete", path: `/api/client/catalog/products/favorite-products/${productId}` },
    { method: "delete", path: `/api/client/catalog/product/favorite-products/${productId}` },
    { method: "put", path: `/api/client/catalog/products/${productId}/unfavorite`, body },
    { method: "put", path: `/api/client/catalog/product/${productId}/unfavorite`, body },
    { method: "post", path: "/api/client/user-preferences", body },
    { method: "post", path: "/api/client/preferences", body },
    { method: "put", path: "/api/client/user-preferences", body },
    { method: "put", path: "/api/client/preferences", body },
  ...
  const candidates = kind === "add" ? defaultAddCandidates : defaultRemoveCandidates;

  if (!configuredPath) return candidates;
  ...
  return [
    ...
    ...candidates.filter((candidate) => candidate.path !== configuredPath),
  ];
}
...
async function runFavoriteMutation(kind, productOrId) {
  ...
  const candidates = getMutationCandidates(kind, productId);
  let lastError = null;

  for (const candidate of candidates) {
    try {
      if (candidate.method === "delete") return await http.delete(candidate.path);
      return await http[candidate.method](candidate.path, candidate.body ?? favoriteBody(productId, kind === "add"));
    } catch (error) {
      lastError = error;
  ...
  throw new ApiError(
    ...
    "Le Swagger fourni expose la lecture des favoris, mais pas l'endpoint officiel d'ajout/suppression. Configure VITE_FAVORITE_PRODUCT_ADD_ENDPOINT et VITE_FAVORITE_PRODUCT_REMOVE_ENDPOINT si le backend les possède.",
    lastError?.data || null
  );
}
