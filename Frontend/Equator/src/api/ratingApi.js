...
export async function rateProduct(productOrId, rating) {
  ...
  if (!configuredPath) {
    throw new ApiError(
      501,
      "Le Swagger fourni expose averageRating/ratingCount, mais pas l'endpoint de soumission d'une note. Configure VITE_PRODUCT_RATING_ENDPOINT si le backend possède cette route.",
      null
    );
  }

  if (!["post", "put", "patch"].includes(method)) {
    throw new ApiError(400, "VITE_PRODUCT_RATING_METHOD doit être POST, PUT ou PATCH.", null);
  }
